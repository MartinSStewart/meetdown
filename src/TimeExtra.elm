...
toUtcIcsString _ posixMs =
    let
        posix = Time.millisToPosix posixMs
        year = Time.toYear Time.utc posix |> String.fromInt
        ...
        day = Time.toDay Time.utc posix |> (\d -> if d < 10 then "0" ++ String.fromInt d else String.fromInt d)
        hour = Time.toHour Time.utc posix |> (\h -> if h < 10 then "0" ++ String.fromInt h else String.fromInt h)
        minute = Time.toMinute Time.utc posix |> (\m -> if m < 10 then "0" ++ String.fromInt m else String.fromInt m)
        second = Time.toSecond Time.utc posix |> (\s -> if s < 10 then "0" ++ String.fromInt s else String.fromInt s)
    in
